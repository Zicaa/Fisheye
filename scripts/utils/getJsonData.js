...
/* //Fonction asynchrone qui récupère les données des médias dans le json grâce à la méthode fetch, à utiliser avec un serveur local (CORS non autorisés)
export async function getMedias() {
    try {
    // Je récupère les données des médias dans le fichier json et les stocke dans une constante
	  	const json = "./data/photographers.json";
        // Je formule ma requête grâce à fetch
		const query = await fetch(json)
		// Je stocke la réponse de ma requête dans ma constante data
